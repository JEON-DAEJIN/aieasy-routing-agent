...
- 분류 결과: `OUT_OF_SCOPE`
- 호출한 근거 문서: (없음 — 범위 밖으로 판정되어 넘김)
- 검증 결과: ✓ 통과 (애초에 근거 없는 주장을 하지 않았으므로 자동 통과)

신경 쓴 부분은 최종 답변 한 줄만 보여주지 않고 **분류 결과 → 호출한 근거 문서 → 검증 결과**를 항상 답변과 함께 노출한 것이다. 케이스 1에서는 "이 답이 어느 문서에서 왔는지"가 바로 보이고, 케이스 2에서는 "왜 답을 못 하는지"(범위 밖 판정)가 답변 문구뿐 아니라 분류 결과 배지로도 이중으로 확인된다 — 이것이 N039 10장이 말하는 관찰가능성이다.
